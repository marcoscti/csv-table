(function ($) {
  $(document).ready(function () {
    $(".csv-table-ajax-wrap").each(function () {
      ...
      var bottomPaginationEl = $(
        '<div class="csv-table-pagination bottom-pagination"></div>',
      ...
      var has_header =
        wrap.data("has_header") === "1" || wrap.data("has_header") === "true";
      var currentPage = 1;
      var isLoading = false;
      ...
      function fetchData() {
        ...
        if (tbody.children().length === 0) {
          tbody.html(
            '<tr><td colspan="10" style="text-align: center; padding: 20px;">Carregando dados...</td></tr>',
          );
        }
        ...
        console.log("Enviando dados para AJAX:", CSVTableAjax);
        $.ajax({
            ...
            if (response.success) {
              ...
              var errorMsg =
                typeof response.data === "string"
                  ...
                  : "Erro desconhecido";
              tbody.html(
                '<tr><td colspan="10" style="color: red; text-align: center;">Erro: ' +
                  errorMsg +
                  "</td></tr>",
              );
              paginationEl.empty();
          ...
          error: function (xhr, status, error) {
            ...
            tbody.html(
              '<tr><td colspan="10" style="color: red; text-align: center;">Erro de conexão, tente novamente mais tarde.</td></tr>',
            );
            paginationEl.empty();
          },
        });
      ...
      function renderTable(data) {
        ...
        if (!data || !data.rows || data.rows.length === 0) {
          tbody.html(
            '<tr><td colspan="10" style="text-align: center;">Nenhum dado encontrado</td></tr>',
          );
      ...
      function updatePagination(data) {
        ...
        $("<div>")
          .addClass("pagination-info")
          .text(infoText)
          .appendTo(bottomPaginationEl);

        if (total_pages <= 1) return;

        var paginationControls = $("<div>").addClass("pagination-controls");
        var bottomPaginationControls = $("<div>").addClass(
          "pagination-controls",
        ); // New controls for bottom
